...
What would you like to know?`,
    ...
    return (
        <div className="max-w-5xl mx-auto p-6 h-screen flex flex-col bg-gray-50 dark:bg-gray-900">
            {/* Header */}
            <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-200 p-6 mb-4">
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h1 className="text-2xl font-black text-gray-900 dark:text-white">AlienGPT</h1>
                            <p className="text-sm text-gray-500 dark:text-gray-400">Your AI Project Assistant</p>
                        </div>
                    </div>
                    <button
                        onClick={handleClear}
                        className="px-4 py-2 bg-gray-100 dark:bg-gray-700 text-gray-700 rounded-lg hover:bg-gray-200 font-semibold text-sm flex items-center gap-2 transition"
                    >
            ...
            {/* Messages */}
            <div className="flex-1 bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-200 p-6 overflow-y-auto mb-4">
                            ...
                            <div className={`flex-1 max-w-3xl ${message.role === 'user' ? 'text-right' : 'text-left'}`}>
                                <div className={`inline-block p-4 rounded-2xl ${message.role === 'user'
                                    ...
                                    {message.role === 'assistant' ? (
                                        <div className="prose prose-sm max-w-none prose-table:border-collapse prose-th:border prose-th:border-gray-300 prose-th:bg-gray-100 dark:bg-gray-700 prose-th:p-2 prose-td:border prose-td:border-gray-300 prose-td:p-2">
                                            <ReactMarkdown
                                                remarkPlugins={[remarkGfm]}
                                                components={{
                                                    table: ({ node, ...props }) => (
                                                        <table className="w-full border-collapse border border-gray-300 dark:border-gray-600 my-2" {...props} />
                                                    ),
                                                    thead: ({ node, ...props }) => (
                                                        <thead className="bg-gray-100" {...props} />
                                                    ),
                                                    th: ({ node, ...props }) => (
                                                        <th className="border border-gray-300 dark:border-gray-600 px-3 py-2 text-left font-bold" {...props} />
                                                    ),
                                                    td: ({ node, ...props }) => (
            ...
            {/* Input */}
            <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-200 p-4">
                <div className="flex gap-3">
                    <textarea
                        value={input}
                        ...
                        className="flex-1 border border-gray-300 dark:border-gray-600 rounded-lg p-3 text-sm focus:ring-2 focus:ring-purple-500 outline-none resize-none"
                        rows={2}
                        disabled={loading}
                    />
                    <button
