...
function NavLink({ to, icon: Icon, label, isCollapsed }) {
  ...
  return (
    <Link to={to} title={isCollapsed ? label : ""} className={`flex items-center gap-3 px-4 py-3 rounded-lg transition font-bold text-sm ${isActive ? 'bg-gray-100 dark:bg-gray-700 text-black dark:text-white' : 'text-gray-500 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-700 hover:text-black dark:hover:text-white'} ${isCollapsed ? 'justify-center px-2' : ''}`}>
...
function AppLayout({ children }) {
  ...
  return (
    <div className="flex min-h-screen bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-gray-100 font-sans">
      <aside className={`${isCollapsed ? 'w-20' : 'w-64'} bg-white dark:bg-gray-800 border-r border-gray-200 dark:border-gray-700 flex flex-col fixed h-full z-10 transition-all duration-300 ease-in-out`}>
        <div className="p-6 border-b border-gray-100 dark:border-gray-700 flex items-center gap-3 overflow-hidden">
          <div className="w-8 h-8 bg-black dark:bg-white rounded-lg flex items-center justify-center text-white dark:text-black font-bold shrink-0">A</div>
          {!isCollapsed && <span className="font-black text-xl tracking-tight whitespace-nowrap dark:text-white">Alien Portal</span>}
        </div>
        ...
        <div className="p-4 border-t border-gray-100 dark:border-gray-700">
            ...
            <div className="mt-4 p-3 bg-gray-50 dark:bg-gray-700 rounded-lg">
              <div className="flex items-center gap-2 mb-2">
                ...
                <div className="flex-1 min-w-0">
                  <div className="font-bold text-sm truncate dark:text-white">{user.display_name || 'User'}</div>
              ...
              <button
                onClick={logout}
                className="w-full flex items-center justify-center gap-2 text-xs font-medium text-gray-500 dark:text-gray-300 hover:text-red-600 dark:hover:text-red-400 py-2 transition"
              >
          ...
          <button
            ...
            className="mt-2 w-full flex items-center justify-center p-2 rounded-lg bg-gray-50 dark:bg-gray-700 hover:bg-gray-100 dark:hover:bg-gray-600 text-gray-400 dark:text-gray-300 hover:text-black dark:hover:text-white transition"
          >
...
function ThemeToggle({ isCollapsed }) {
  ...
  return (
    <button
      ...
      className={`mt-2 w-full flex items-center p-2 rounded-lg bg-gray-50 dark:bg-gray-700 hover:bg-gray-100 dark:hover:bg-gray-600 text-gray-500 dark:text-gray-300 hover:text-black dark:hover:text-white transition ${isCollapsed ? 'justify-center' : 'gap-2 px-4'}`}
    >
