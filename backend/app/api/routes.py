...
def delete_project(project_id):
    ...
    try:
        ...
        admin_db.table("projects").delete().eq("id", project_id).execute()
...
@api.route('/projects/<project_id>/stakeholders', methods=['GET'])
@require_auth
def get_project_stakeholders(project_id):
    ...
    """
    try:
        # Get stakeholders with contact details using Supabase query
        result = db.table("project_stakeholders").select(
            "id, role, contact_id, contacts(id, name, email, phone, company, role, slack_user_id)"
...
@api.route('/projects/<project_id>/stakeholders', methods=['POST'])
@require_auth
...
def add_project_stakeholder(project_id):
    ...
    try:
        ...
        result = db.table("project_stakeholders").insert({
            "project_id": project_id,
...
@api.route('/projects/<project_id>/stakeholders/<contact_id>', methods=['DELETE'])
@require_auth
@require_role('superadmin', 'internal')
def remove_project_stakeholder(project_id, contact_id):
    """
    Remove a stakeholder from a project.
    """
    try:
        db.table("project_stakeholders").delete().eq("project_id", project_id).eq("contact_id", contact_id).execute()
        return jsonify({"success": True, "message": "Stakeholder removed"})
    except Exception as e:
        print(f"Error removing stakeholder: {e}")
        return jsonify({"error": str(e)}), 500


@api.route('/projects/<project_id>/sync-stakeholders', methods=['POST'])
@require_auth
@require_role('superadmin', 'internal')
def sync_project_stakeholders(project_id):
    """
    Auto-sync stakeholders from project's Slack channels.
    """
    from slack_sdk import WebClient
    from app.core.config import settings
    ...
    try:
        ...
        # Get members from both channels
        for channel_key in ['channel_id_internal', 'channel_id_external']:
            channel_id = project_data.get(channel_key)
            if channel_id:
                try:
        ...
        # Find contacts with these Slack IDs
        added_count = 0
        for user_id in all_user_ids:
            contact = db.table("contacts").select("id").eq("slack_user_id", user_id).execute()
            if contact.data:
                contact_id = contact.data[0]['id']
                # Add to stakeholders (ignore if already exists)
                try:
                    db.table("project_stakeholders").insert({
                        "project_id": project_id,
                        "contact_id": contact_id,
                        "added_by": g.user['id']
                    }).execute()
        ...
        return jsonify({
            "success": True,
            "message": f"Synced {added_count} stakeholders from channels",
            "added": added_count
        })
    except Exception as e:
        print(f"Error syncing stakeholders: {e}")
        return jsonify({"error": str(e)}), 500
...
def create_project():
    ...
    Slack channels can be added manually or via Scanner.
    """
    data = request.json
    client_name = data.get("client_name")
    ...
    try:
        # Create DB Entry (without Slack channels)
        project_data = {
        ...
        for name in [internal_channel_name, external_channel_name]:
            try:
                ...
                if e.response["error"] == "name_taken":
                    ...
                    pass  # Continue without Slack channels
